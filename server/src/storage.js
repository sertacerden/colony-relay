...
export function validateSave(record) {
  ...
  if (record.schemaVersion === 1 && [1,2].includes(record.levelVersion) && Number.isInteger(record.sector) && record.sector >= 0 && record.sector < (record.levelVersion===1?3:25) && record.roster) {
  ...
  if (record.schemaVersion !== 1 || record.levelVersion !== LEVEL_VERSION
    ...
    || !record.roster || typeof record.roster !== 'object') throw new Error('Kayıt bu oyun sürümüyle uyumlu değil');
  return record;
}
